...
Local agents, Codex, and Claude Code share the same pattern: read root guidance, query the frontier, claim one lease, edit role-owned files, and record the result through the CLI. The local SQLite index is generated automatically from `state/current.json`.
...
ChatGPT Work receives a generated execution projection rather than the full state archive. A pack contains at most the requested frontier limit, linked context, relevant role prompts, project core files, and referenced artifacts.
