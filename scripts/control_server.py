...
<article><h3>In flight <b>{len(leases)}</b></h3><ul>{_rows(leases, lambda x:x['leased_by'], lambda x:f"{x['role']} · until {x['expires_at']}")}</ul></article>
<article><h3>Verification <b>{len(verification)}</b></h3><ul>{_rows(verification, lambda x:x['title'], lambda x:f"waiting for {x['verifier_role']}")}</ul></article>
