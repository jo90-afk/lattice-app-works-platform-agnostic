...
def render_html(model: dict) -> str:
    ...
    if inbox_items:
        inbox_rows = "".join(
            "<li><strong>" + html.escape(str(item["title"])) + "</strong><span>" +
            html.escape(
                str(item["project_id"]) + " · " + str(item["kind"]) +
                (" · " + str(item["severity"]) if item.get("severity") else "")
            ) + "</span></li>"
            for item in inbox_items
        )
    else:
        inbox_rows = "<li class='empty'>No Principal decision required.</li>"
    ...
    for item in model["projects"]:
        ...
        def rows(values, primary, secondary):
            ...
            return "".join(
                f"<li><strong>{html.escape(str(primary(v)))}</strong>"
                f"<span>{html.escape(str(secondary(v)))}</span></li>"
                for v in values
            )
...
</style></head><body><main><div class='top'><div><p class='eyebrow'>Local control surface</p><h1>Lattice</h1><p class='lede'>Current project state, derived work, verification, and exceptions. This surface is read-only; authority remains in the guarded state engine.</p></div><div class='revision'>revision {model['revision']}</div></div><section class='principal'><header><div><p class='eyebrow'>Human decision boundary</p><h2>Principal inbox</h2></div><div class='count'>{inbox['count']} open · {inbox['blocking_count']} blocking</div></header><ul>{inbox_rows}</ul></section>{body}</main></body></html>"""
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        try:
            if parsed.path == "/api/state":
                ...
                payload = render_html(self._model()).encode()
                status = 200
