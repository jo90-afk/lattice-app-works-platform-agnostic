...
def dispatch_hooks(root: Path, event_type: str, envelope: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for index, command in enumerate(commands):
        completed = subprocess.run(command, cwd=root, input=payload, text=True, capture_output=True, check=False)
        result = {"index": index, "command": command, "returncode": completed.returncode, "stdout": completed.stdout, "stderr": completed.stderr}
        results.append(result)
        if completed.returncode != 0:
            raise LatticeError(f"Lifecycle hook failed for {event_type} at index {index}: " + (completed.stderr.strip() or completed.stdout.strip() or f"exit {completed.returncode}"))
    return results
