#!/usr/bin/env python3
"""Adversarial 0.0.8 scenarios for authority, truth invalidation, and portfolio capacity."""
...
def _ambiguous_requirements(store: StateStore, run_id: str, host: str, _folder: Path) -> dict[str, Any]:
    ...
    # Evaluation-only time injection: make blocked-information time measurable without sleeping.
    ...
    workers = [Worker("application-available", "application", host, "application-workspace")]
    plan = candidate_plan(store, workers, limit=5)
    blocked_seconds = max(0.0, (observed_now - datetime.fromisoformat(blocked_since.replace("Z", "+00:00"))).total_seconds())
    passed = bool(
        frontier
    ...
    return _result(
        ...
        blocked_seconds_missing_information=blocked_seconds,
    )


def _contradictory_information(store: StateStore, run_id: str, host: str, _folder: Path) -> dict[str, Any]:
    started = time.perf_counter()
    project_id = "eval-contradiction"
    truth_id = "eval-original-truth"
    _objective, milestone_id, condition_id = _seed_project(
        store, project_id, truth_id=truth_id, condition_id="eval-contradiction-condition"
    )
    context = 0
    ...
    accepted_link = store.conn.execute(
    ...
    passed = bool(
        ...
        and accepted_link["accepted_truth_version"] is None
        and any(action["target_id"] == condition_id for action in frontier)
        and store.conn.execute("SELECT status FROM milestones WHERE id = ?", (milestone_id,)).fetchone()["status"] == "active"
    )
    return _result(
        store,
...
def _multi_project_contention(store: StateStore, run_id: str, host: str, folder: Path) -> dict[str, Any]:
    started = time.perf_counter()
    ...
    context = sum(len(json.dumps(claim["action"], sort_keys=True).encode("utf-8")) for claim in first["claims"])
    ...
    context += sum(len(json.dumps(claim["action"], sort_keys=True).encode("utf-8")) for claim in second["claims"])
...
SCENARIOS: dict[str, Callable[[StateStore, str, str, Path], dict[str, Any]]] = {
    "ambiguous-requirements-escalation": _ambiguous_requirements,
    "contradictory-new-information": _contradictory_information,
    "multi-project-capacity-contention": _multi_project_contention,
}


def run_adversarial_scenario(
    scenario_id: str, *, host: str = "local", run_id: str | None = None
) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="lattice-adversarial-eval-") as temporary:
        ...
        with StateStore(ROOT, db_path=folder / "state.db", snapshot_path=folder / "state.json") as store:
            result = SCENARIOS[scenario_id](store, resolved_run_id, host, folder)
...
def main() -> int:
    ...
    try:
        print(
            json.dumps(
                run_adversarial_scenario(args.scenario, host=args.host, run_id=args.run_id),
                indent=2,
                sort_keys=True,
            )
        )
